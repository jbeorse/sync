  ...
  private static final String LOGTAG = AccountInfoActivity.class.getSimpleName();
	...
	protected void onResume() {
		...
		if (request == null) {
			...
			Account account = (Account) intent.getExtras().get(
					INTENT_EXTRAS_ACCOUNT);
			request = accountManager.getAuthToken(account, authString, null,
					this, new AuthTokenCallback(), null);
				...
				AccountInfoActivity.this.runOnUiThread(new Runnable() {
					...
					public void run() {
						Toast.makeText(AccountInfoActivity.this,
								"Authentication Cancelled: " + e.toString(),
	...
	protected Dialog onCreateDialog(int id) {
		...
		case WAITING_ID:
			...
			builder.setMessage(getString(R.string.waiting_auth)).setCancelable(
					false);
