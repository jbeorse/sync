...
public class AggregateDownloadTableActivity extends ListActivity {
	...
	private static final String TAG = AggregateDownloadTableActivity.class
			.getSimpleName();
	...
	private void respondToTablesQuery(List<TableResource> tablesFromServer) {
		if (tablesFromServer == null) {
			finishDialog
					.setMessage(getString(R.string.error_contacting_server));
			finishDialog.show();
	...
	private void initializeDialogs() {
		finishDialog = new AlertDialog.Builder(
				AggregateDownloadTableActivity.this);
		finishDialog.setCancelable(false);
		finishDialog.setPositiveButton(getString(R.string.ok),
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		String dbTableNameActive = NameUtil.createUniqueDbTableName(this,
				appName, tableId);

		try {
			SyncPreferences prefs = new SyncPreferences(this, appName);

			DownloadTableTask task = new DownloadTableTask(prefs.getAccount(),
					tableId, dbTableNameActive, displayName, tableDefinitionUri);
			task.execute();
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	private class GetTablesTask extends
			AsyncTask<Void, Void, List<TableResource>> {
		private final String appName;
		private final String aggregateUrl;
		private final String authToken;
		private ProgressDialog pd;

		public GetTablesTask(String appName, String aggregateUrl,
				String authToken) {
		...
		protected List<TableResource> doInBackground(Void... args) {
			...
			Synchronizer sync;
			try {
				sync = new AggregateSynchronizer(appName, aggregateUrl,
						authToken);
			...
			// we may want to download data from the server even if we already
			// have data locally.
			// // filter tables to remove ones already downloaded
			// if (tables != null) {
			// DbHelper dbh =
			// DbHelper.getDbHelper(AggregateDownloadTableActivity.this,
			// appName);
			// // we're going to check for downloaded tables ONLY in the server
			// store,
			// // b/c there will only be UUID collisions if the tables have been
			// // downloaded, which means they must be in the server KVS. The
			// // probability of a user defined table, which would NOT have
			// entries
			// // in the server KVS, having the same UUID as another table is
			// // virtually zero.
			// TableProperties[] props =
			// TableProperties.getTablePropertiesForDataTables(dbh,
			// KeyValueStore.Type.SERVER);
			// for (TableProperties tp : props) {
			// for ( int i = 0 ; i < tables.size() ; ++i ) {
			// TableResource table = tables.get(i);
			// if ( table.getTableId().equals(tp.getTableId()) ) {
			// SyncTag serverTag = new SyncTag(table.getDataETag(),
			// table.getPropertiesETag(), table.getSchemaETag());
	...
	private class DownloadTableTask extends AsyncTask<Void, Void, Void> {
		...
		public DownloadTableTask(String accountName, String tableId,
				String dbTableName, String displayName,
				String tableDefinitionUri) {
		...
		protected Void doInBackground(Void... params) {
			...
			try {
				synchronizer = new AggregateSynchronizer(appName, aggregateUrl,
						authToken);
			...
			SyncProcessor processor = new SyncProcessor(
					...
					new SyncResult());
			// We're going to add a check here for the framework directory. If
			// we
			// don't have it, you also have to sync app level files the first
			// time.
			...
			try {
				...
				Log.e(TAG,
						"ResourceAccessException trying to pull app-level files for the "
								+ "first time during table download.");
				// TODO report failure properly
				e.printStackTrace();
				// It's not going to be deemed fatal at this point if the app
				// folder doesn't pull down. Perhaps eventually it should, but
				// not
				// for now.
				return null;
			}
		...
		protected void onPostExecute(Void result) {
			...
			if (downloadResult == null) {
				finishDialog.setMessage(getString(
						R.string.failed_download_table, displayName,
						"Unknown Error"));
				...
				finishDialog.setMessage(getString(R.string.downloaded_table,
						displayName));
			}
			finishDialog.show();
		}
